...
public class ArchiveImpl1 implements Archive1 {

    // этот компаратор позволяет сортировать объекты типа Document сначала по дате,
    // потом по ID документа
    static Comparator<Document1> comparator = (d1, d2) -> {
        int res = d1.getDate().compareTo(d2.getDate());
        return res != 0 ? res : Integer.compare(d1.getDocumentId(), d2.getDocumentId());
    };

    //fields
    private Document1[] documents;  // хранение документов в массиве
    private int size; // количество документов

    // constructor
    public ArchiveImpl1(int capacity) {
        documents = new Document1[capacity];
    }

    @Override
    public boolean addDocument(Document1 document) {
        if (document == null || size == documents.length || getDocFromFolder(document.getFolderId(),
    ...
    public boolean updateDocument(int folderId, int documentId, String url) {
        Document1 document = getDocFromFolder(folderId, documentId);
        if (document == null) {
            return false;
        }
    ...
    @Override
    public Document1 getDocFromFolder(int folderId, int documentId) {
        Document1 pattern = new Document1(folderId, documentId, null, null, null);
        for (int i = 0; i < size; i++) {
    ...
    @Override
    public Document1[] getAllDocFromFolder(int folderId) {
    ...
    private Document1[] findByPredicate(Predicate<Document1> predicate) { // возвращаем массив найденных объектов
        Document1[] res = new Document1[size]; // объявили массив избыточной длины
    ...
    @Override
    public Document1[] getDocBetweenDate(LocalDate dateFrom, LocalDate dateTo) {
        Document1 pattern = new Document1(0, Integer.MIN_VALUE, null, null, dateFrom.atStartOfDay());  // почему Integer.MIN_VALUE, а не 0
        ...
        pattern = new Document1(0, Integer.MAX_VALUE, null, null, LocalDateTime.of(dateTo, LocalTime.MAX));// находим
        // правый край
